...
class GraphProgram:
    ...
    def to_json(self) -> str:
        """Convert the task to JSON for storage."""
        data = {"tensor": self.tensor, "placeholders": self.placeholders}
        return jsonpickle.encode(data)

    @classmethod
    def from_json(cls, json_str: str) -> Union["GraphProgram", ValueError]:
        """
        Import a task from JSON.

        Args:
            json_str: JSON string containing tensor and placeholder data
        ...
        """
        try:
            unpickled = jsonpickle.decode(json_str)
            result = cls._sanity_check(unpickled)
            if isinstance(result, ValueError):
                return result
            return cls(**unpickled)
        except Exception as e:
            return ValueError(f"Failed to decode JSON data: {str(e)}")
